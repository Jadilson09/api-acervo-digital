...
class Livro {
  ...
  try {
    // Query SQL com colunas explícitas buscando todos os livros com status ativo
    ...
    // .map() transforma cada linha retornada pelo banco em um objeto LivroDTO
  ...
  try {
    // Query SQL com colunas explícitas buscando um livro específico pelo ID
    ...
    // Verifica se algum livro foi encontrado antes de tentar acessar os dados
    ...
    const livroDTO: LivroDTO = {
      ...
      autor:                   row.autor,
      editora:                 row.editora,
      ano_publicacao:          row.ano_publicacao,          // Ano de publicação
      isbn:                    row.isbn,                    // ISBN
      quant_total:             row.quant_total,             // Quantidade total
      quant_disponivel:        row.quant_disponivel,        // Quantidade disponível
      quant_aquisicao:         row.quant_aquisicao,         // Quantidade de aquisição
      valor_aquisicao:         row.valor_aquisicao,         // Valor de aquisição
      status_livro_emprestado: row.status_livro_emprestado, // Status de empréstimo
      status_livro:            row.status_livro             // Status ativo/inativo
    };
  ...
  try {
    ...
    // Usa rowCount para verificar se o INSERT afetou alguma linha
    // É mais confiável que rows.length pois reflete diretamente o resultado da operação
  ...
  try {
    ...
    // Se o livro não existir (null) ou já estiver inativo, interrompe e retorna false
    ...
    // Executa a desativação dos empréstimos do livro
    ...
    // Se rowCount for maior que 0, pelo menos uma linha foi desativada — operação bem-sucedida
  ...
  try {
    ...
    // Se o livro não existir (null) ou estiver inativo, interrompe e retorna false
    // "Early return" evita aninhamento desnecessário com if/else
