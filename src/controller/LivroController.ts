...
class LivroController extends Livro {
 ...
 static async todos(req: Request, res: Response): Promise<void> {
  try {
    ...
    if (!listaDeLivros) {
      res.status(400).json({ mensagem: "Não foi possível recuperar as informações dos livros." });
      return;
    }
    ...
    res.status(500).json({ mensagem: "Erro ao recuperar as informações dos livros." });
  }
}
  ...
  try {
    ...
    const novoLivro = new Livro(
      ...
      // .toString() é necessário pois o construtor de Livro espera uma string, não um número
    ...
    if (result) {
      ...
      // ⚠️ Corrigido: era 200 (OK), o correto para criação de recursos é 201
      res.status(201).json({ mensagem: "Livro cadastrado com sucesso." });
    } else {
      // Retorna status 400 (Bad Request) se o banco não conseguiu salvar
      res.status(400).json({ mensagem: "Não foi possível cadastrar o livro no banco de dados." });
    }
  ...
  try {
    ...
    if (result) {
      ...
      // ⚠️ Corrigido: era 201 (Created), o correto para remoção é 200
      res.status(200).json({ mensagem: "Livro removido com sucesso." });
    } else {
      // Retorna status 404 (Not Found) se o livro não foi encontrado ou já estava inativo
      res.status(404).json({ mensagem: "Livro não encontrado para exclusão." });
    }
    ...
    // Retorna status 500 (Internal Server Error) para erros inesperados do servidor
    res.status(500).json({ mensagem: "Erro ao remover o livro." });
  }
}
